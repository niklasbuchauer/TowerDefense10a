...
public class Game extends Canvas implements Runnable
{
    ...
    private void drawMenu(Graphics g)
    {
        ...
        g.drawString("TOWER DEFENSE", 238, 232);

        g.setFont(new Font("Dialog", Font.PLAIN, 18));
        g.drawString("ENTER oder Klick zum Starten", 248, 278);

        g.setFont(new Font("Dialog", Font.PLAIN, 14));
        g.drawString("1 Basic 60$   2 Sniper 100$   3 Rapid 80$   4 Freeze 90$", 158, 336);
        g.drawString("Linksklick setzt Turm   Rechtsklick hebt Auswahl auf", 176, 360);
    }

    private void drawGameOver(Graphics g)
    {
        g.setColor(Color.RED);
        g.setFont(new Font("Dialog", Font.BOLD, 52));
    ...
    {
        if (state != GameState.PLAYING || selectedTower == 0)
            return;
        ...
        int range = getSelectedTowerRange();
        g.setColor(new Color(0, 150, 255, 40));
        g.fillOval(mousePoint.x - range, mousePoint.y - range, range * 2, range * 2);
        g.setColor(new Color(0, 150, 255, 120));
        g.drawOval(mousePoint.x - range, mousePoint.y - range, range * 2, range * 2);
    }

    private int getSelectedTowerRange()
    {
        switch (selectedTower)
        {
            case 1: return 120;
